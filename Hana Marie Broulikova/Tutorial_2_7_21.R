...
mi[[1]][["loggedEvents"]]
mi[[2]][["loggedEvents"]]
Sys.time() - mi_time # calculate time to run the analysis by extracting p from the current system time  
...
#### Bootstrapping ####
# create lists to again split by treatment arm
...
fsur <- function(impds, i){
  dscc2 <- impds[i,] 
  r1 <- Tcosts ~ trt + cT0 + age + gen + educ + smok + com 
...
# extracts the confidence intervals estimated by bca (to be used to estimate CI for costs)
...
for (wtp in seq(0, 50000, 1000)) {
  propPos <- results[, mean((wtp * booteffect_diff - bootcost_diff) > 0)] 
...
CEACs_plot <- ggplot(CEAC, aes(x = wtp, y = propPos, color = Scenario)) + 
  ...
  labs(title="2b") +
  xlab("Willingness to pay threshold (Euros/QALY)") +
